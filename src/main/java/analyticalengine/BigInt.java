 ...
 * Science. Permission to copy this software, to redistribute it, and to use it
 ...
 * 
 * 4. MIT has made no warrantee or representation that the operation of this
 * software will be error-free, and MIT is under no obligation to provide any
 ...
 * shall be no use of the name of the Massachusetts Institute of Technology nor
 * of any adaptation thereof in any advertising, promotional, or sales
 ...
 * <p>
 * The BigInt class implements integers with unbounded precision. BigInts
 * support the Classical Algorithms of addition, subtraction, multiplication
 * and division giving a quotient and remainder.
 * 
 * <p>
 * BigInts are a subclass of <tt>java.lang.Number</tt> so they support the
 * conversion operations <tt>intValue()</tt>, <tt>doubleValue</tt> etc., just
 * like the other forms of `boxed' number (classes Integer, Float, Long and
 * Double). It is a pity that `Integer' was used as the name for a boxed
 * <tt>int</tt>. `Int' would have been a more regular name, and it would have
 * left the name `Integer' open for multiple precision integers which better
 * model the mathematical notion of integer.
 * 
 * <p>
 * The algorithms are taken from Knuth, Donald E.,
 ...
 * 
 * <p>
 * BigInts behave like numbers in that they are never modified after creation.
 * For example
 * 
 * <pre>
 ...
 * <p>
 * Operations do not always return new BigInt objects. For example, when adding
 * zero to a thousand digit number, it is more efficient to return the thousand
 * digit number than to make a copy. You should never rely on Java's pointer
 * equality operator (<tt>==</tt>) to compare BigInts in any way.
 * 
 * Implementing these algorithms in Java is quite slow. Ideally, the core
 * methods should be native methods. A Just-In-Time compiler would help, but
 * unless it could lift array bounds checking out of loops, native methods
...
public final class BigInt extends Number {
     ...
     * <p>
     * Note: <tt>equals</tt> will return false if obj is not a BigNum, even if
     * obj makes sense as an integer:
     * 
     * <pre>
     ...
     * 
     * @param comparand
     *            a BigInt to compare against.
     * @return -1 if less than comparand, 0 if equal to comparand, and 1 if
    ...
    /**
     * Divide two BigInts, producing the quotient and remainder. This is more
     * efficient than computing the values separately.
     * 
     * @return an array of two BigInts, with the quotient at index 0, and the
     *         remainder at index 1.
     */
    ...
    public static BigInt[] divide(BigInt numerator, BigInt denominator) {
        ...
        if (numerator.isZero()) {
            ...
            boolean q_negative_p = denominator.negative
                                                       ? (!r_negative_p)
                                                           : r_negative_p;
            switch (compare_unsigned(numerator, denominator)) {
            case EQUAL:
                results[0] = q_negative_p
                                         ? NEGATIVE_ONE
                                             : ONE;
                results[1] = ZERO;
                break;
            ...
            case GREATER:
                if (denominator.last == 0) {
                    ...
                    if (digit == 1) {
                        ...
                        results[0] = divide_unsigned_small_denominator(
                                numerator, digit, results, q_negative_p,
                                r_negative_p);
                } else
                    divide_unsigned_large_denominator(numerator, denominator,
                            results, results, q_negative_p, r_negative_p);
            }
        }
        return results;
    }
        ...
        case GREATER:
            if (denominator.last == 0) {
                ...
                if (digit == 1) {
                    ...
                    return divide_unsigned_small_denominator(numerator, digit,
                            null, q_negative_p, false);
            } else {
                BigInt[] results = new BigInt[2];
                divide_unsigned_large_denominator(numerator, denominator,
                        results, null, q_negative_p, false);
                return results[0];
            }
        default:
        ...
        case GREATER:
            if (denominator.last == 0) {
                ...
                if (digit == 1)
                    ...
                    return numerator
                            .remainder_unsigned_small_denominator(digit);
            } else {
                BigInt[] results = new BigInt[2];
                divide_unsigned_large_denominator(numerator, denominator,
                        null, results, false, numerator.negative);
                return results[1];
            }
        default:
            throw new Error("Impossible to get here");
    ...
    static BigInt multiply_unsigned_small_factor(BigInt x, int f,
            boolean negative) {
    ...
    static int scale_up(int[] from, int last, int[] to, int factor,
            int carry_in) {
    ...
    static BigInt divide_unsigned_small_denominator(BigInt numerator,
            int denominator, BigInt[] remainder_ref, boolean q_negative_p,
    ...
    static void divide_unsigned_large_denominator(BigInt numerator,
            BigInt denominator, BigInt[] quotient_ref, BigInt[] remainder_ref,
            boolean q_negative_p, boolean r_negative_p) {
        int last_n = numerator.last + 1;
        int last_d = denominator.last;
        BigInt q = (quotient_ref == null)
                                         ? null
                                             : allocate(last_n - last_d,
                                                     q_negative_p);
    ...
    static void divide_unsigned_normalized(BigInt u, BigInt v, BigInt q) {
        ...
        for (int j = u.last; j > vlast; j--) {
            ...
            int qj = divide_subtract(vdigits, vlast, guess, udigits, j - vlast
                    - 1);
            if (q != null)
                q.digits[qi--] = qj;
        }
    ...
    static int divide_subtract(int[] vdigits, int vlast, long guess,
        ...
        // Subtraction generated a carry, so the guess is off by one. Add v
        // back
     ...
     *                The String cannot be parsed as an integer.
     */
    public static BigInt valueOf(String s, int radix)
    ...
    public int intValue() {
        ...
        for (int i = last; i >= 0; i--) {
            n = (n << BITS) + digits[i];
            if (n > limit)
                throw new ArithmeticException("too big to convert to int: "
                        + this + " limit " + limit);
        }
        return (int) (negative
                              ? -n
            ...
            if (ov != 0) {
                if (negative && ov == 1 && i == 0 && n == 0x8000000000000000L)
                    ...
                    throw new ArithmeticException(
                            "Too big to convert to long: " + this);
            }
        }
        return negative
                       ? -n
    ...
    /**
     * Returns the value of the number as a float. This may involve loss of
    ...
    /**
     * Returns the value of the number as a double. This may involve loss of
     * precision or overflow.
     */
    public double doubleValue() {
        ...
        // double.
        double n = 0.0;
        int limit = last > DOUBLE_USEFUL_WORDS
                                              ? last - DOUBLE_USEFUL_WORDS
                                                  : 0;
     ...
     * 
     * @param rng
     *            the random number generator to use. It is called an
     *            unspecified number of times.
     * @param limit
     *            specifies the range of the result.
     ...
     * BigInt limit) { if (limit.negative || limit.isZero()) throw new
     * ArithmeticException ("Random limit must be >0"); int[] digits = new
     * int[limit.last+1]; int i = limit.last; while (i>=0) { // generate most
     * significant digits int lead = limit.digits[i], mask = 1, trial; while
     * (mask<lead) mask=(mask<<1)+1; do { trial = mask; for (int j = i; j>=0;
     * j--) trial &= rng.nextInt(); } while (trial>lead); digits[i--] = trial;
     * if (trial<lead) break; } for ( ; i>=0; ) { int trial = MASK; for (int j
     * = i; j>=0; j--) trial &= rng.nextInt(); digits[i--] = trial; } return
     * new BigInt (false, digits, limit.last).trim(); }
     */
}
